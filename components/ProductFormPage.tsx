...
export const ProductFormPage: React.FC = () => {
  ...
  const handleStockAction = (index: number, action: number) => {
    ...
    const newValue = Math.max(0, current + action);
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-12">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => navigate('/products')} className="p-2">
            <ArrowLeft size={20} />
          </Button>
          <div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="w-full">
           <Card className="p-6 border-0 shadow-sm bg-white dark:bg-zinc-900">
              <h3 className="text-sm font-bold text-zinc-400 uppercase mb-6 flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-2">
                <Tag size={16} className="text-zinc-400" /> Identificação Geral
              </h3>
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full px-4 py-2.5 border border-zinc-200 dark:border-zinc-700 rounded-xl bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white focus:ring-2 focus:ring-zinc-500 outline-none transition-all resize-y min-h-[120px]"
                  />
                </div>
              </div>
           </Card>
        ...
        <div className="w-full space-y-6">
           <Card className="p-6 border-0 shadow-sm bg-white dark:bg-zinc-900 h-full flex flex-col">
              ...
              <div className="hidden lg:grid grid-cols-12 gap-3 px-4 mb-2">
                ...
                <div className="col-span-1"><label className="text-[10px] font-bold text-zinc-400 uppercase">Custo (R$)</label></div>
                <div className="col-span-1"><label className="text-[10px] font-bold text-zinc-400 uppercase">Venda (R$)</label></div>
                <div className="col-span-2"><label className="text-[10px] font-bold text-zinc-400 uppercase text-center">Estoque</label></div>
                <div className="col-span-2"><label className="text-[10px] font-bold text-zinc-400 uppercase">SKU</label></div>
                <div className="col-span-2"><label className="text-[10px] font-bold text-zinc-400 uppercase">EAN / Cód.</label></div>
                ...
                {variants.map((v, index) => (
                  <div key={index} className="relative p-3 lg:p-2 rounded-xl bg-zinc-50/50 dark:bg-zinc-800/30 border border-zinc-100 dark:border-zinc-800 group hover:border-zinc-300 dark:hover:border-zinc-700 transition-all">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-center">
                      ...
                      {/* Estoque */}
                      <div className="lg:col-span-2">
                        <label className="lg:hidden text-[10px] font-bold text-zinc-400 uppercase mb-1 block">Estoque Inicial</label>
                        <div className="flex items-center">
                          ...
                          <input 
                            ...
                            className="w-full h-8 px-1 text-center text-sm border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-0 outline-none font-bold"
                          />
                          <button 
                            type="button" 
                            onClick={() => handleStockAction(index, 1)}
                      ...
                      {/* EAN */}
                      <div className="lg:col-span-2">
                        <label className="lg:hidden text-[10px] font-bold text-zinc-400 uppercase mb-1 block">Código de Barras</label>
                        <input 
                          type="text" 
                          name="ean" 
                          value={v.ean} 
                      ...
                      <div className="lg:col-span-1 flex justify-end">
                        {variants.length > 1 && (
                          <button 
                            ...
                            className="p-1.5 text-zinc-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-md transition-all sm:static absolute top-2 right-2"
              ...
              <div className="mt-8 pt-6 border-t border-zinc-100 dark:border-zinc-800 flex justify-end">
                <Button onClick={handleSubmit} disabled={loading} className="px-10 h-11 text-sm font-bold shadow-lg shadow-emerald-500/20 active:scale-95 transition-transform flex items-center gap-3">
                   {loading ? <Loader2 className="animate-spin" size={20} /> : <><Check size={22} /> {id ? 'Finalizar Edição' : 'Cadastrar Produto'}</>}
                </Button>
              </div>
           </Card>
        </div>
