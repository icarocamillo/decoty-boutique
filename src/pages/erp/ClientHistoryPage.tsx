...
import { RecentSales } from '@/pages/erp/RecentSales';
import { Badge } from '@/components/ui/Badge';
import { formatDateStandard } from '@/utils';
import { useAuth } from '@/contexts/AuthContext';
import { GiftCardAdjustmentModal } from '@/components/erp/GiftCardAdjustmentModal';
import { CrediarioPaymentModal } from '@/components/erp/CrediarioPaymentModal';
...
export const ClientHistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    if (clientId && client && clientId !== client.ui_id.toString()) {
      navigate(`/erp/clients/${client.ui_id}/history`, { replace: true });
    }
  ...
  if (!client) {
    return (
      <div className="p-8 text-center">
        ...
        <Button onClick={() => navigate('/erp/clients')}>Voltar</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate('/erp/clients')}>
          <ArrowLeft size={20} />
        </Button>
        <div>
